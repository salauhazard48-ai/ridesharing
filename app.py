...
def search():
    if request.method == 'POST':
        ...
        raw_trips = Trip.query.filter_by(
            origin=origin, destination=destination, status='active'
        ).all()
        ...
        results = match_trips(
            origin, destination, preferred_time,
            seats_needed, max_budget, available_trips, distance
        )

        return render_template('results.html',
            results=results, path=path, distance=distance,
